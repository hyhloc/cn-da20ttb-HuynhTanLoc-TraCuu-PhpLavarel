...
class HotelController extends Controller
{
    ...
    {
        $this->module_name = 'dịch vụ lưu trú';
    ...
    public function index(Request $request)
    {
        ...
        return view('admin.hotels.index',compact('data','table','status'));
    }
    ...
    public function create()
    {
        if(Auth::guard('admin')->check()){
            ...
            $form = new MyForm();
            $data_form[] = $form->text('name','',1,'Tên cơ sở lưu trú','VD: Khách sạn Cồn Phụng');
            $data_form[] = $form->text('slug','',1,'Slug tên cơ sở (Url)','VD: khach-san-con-phung');
            ...
            $data_form[] = $form->text('price','',0,'Giá lưu trú');
            $data_form[] = $form->text('type','',0,'Loại hình lưu trú','VD: Khách sạn');
    ...
    {
        $this->validate_form($request,'name',1,'Bạn chưa nhập tên cơ sở lưu trú',1,'Tên đã được sử dụng');
    ...
    public function edit(string $id)
    {
        if(Auth::guard('admin')->check()){
            ...
            $form = new MyForm();
            $data_form[] = $form->text('name',$data_edit->name,1,'Tên cơ sở lưu trú','VD: Khách sạn Cồn Phụng');
            $data_form[] = $form->text('slug',$data_edit->slug,1,'Slug tên cơ sở (Url)','VD: khach-san-con-phung');
            ...
            $data_form[] = $form->text('price',$data_edit->price,0,'Giá lưu trú');
            $data_form[] = $form->text('type',$data_edit->type,0,'Loại hình lưu trú','VD: Khách sạn');
            $data_form[] = $form->text('times',$data_edit->times,0,'Thời gian hoạt động','VD: 06:00 - 23:00');
